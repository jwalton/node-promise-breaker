import pb from '..';
import * as pbStar from '..';

const make0 = pb.make((cb: (err?: Error, result?: string) => void) => cb(undefined, 'hello'));
...
const make3 = pb.make(
    (a: string, b: number, c: string, cb: (err?: Error, result?: string) => void) =>
        cb(undefined, 'hello' + a)
);
make3('jason', 2, 'foo').then((result) => console.log(result));

const make4 = pb.make(
    (a: string, b: number, c: string, d: number, cb: (err?: Error, result?: string) => void) =>
        cb(undefined, 'hello' + a)
);
make4('jason', 2, 'foo', 10).then((result) => console.log(result));

const makeStar = pbStar.make((cb: (err?: Error, result?: string) => void) =>
    cb(undefined, 'hello')
);
makeStar().then((result) => console.log(result));
